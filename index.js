document.addEventListener("DOMContentLoaded", () => {
  ...
  function showApp(user){
    ...
    state.balance=user.balances[local.code];
    state.active=JSON.parse(localStorage.getItem(`otphub_active_${user.id}`)||"null");
    els.authScreen.classList.add("hidden"); els.app.classList.remove("hidden");
    els.userEmail.textContent=user.email; render(); startTimer();
  }

  function render(){
    els.walletBalance.textContent=money(state.balance);
    els.heroPrice.textContent=money(local.price);
    els.modalCountry.textContent=`${local.flag} ${local.currency}`;
    els.modalBalance.textContent=money(state.balance);
    els.servicesTitle.textContent=`Services - ${money(local.price)} • ${selected.flag} ${selected.name}`;
    els.phoneCountryRow.innerHTML="";
    PHONE_COUNTRIES.filter(c=>c.name.toLowerCase().includes(state.search.toLowerCase())).forEach(c=>{
      const b=document.createElement("button"); b.className="country-chip"+(c.code===selected.code?" active":""); b.textContent=`${c.flag} ${c.name} ${c.prefix}`;
      b.onclick=()=>{selected=c; render();}; els.phoneCountryRow.appendChild(b);
    });
    ...
    local.topups.forEach((a,i)=>{
      const o=document.createElement("div"); o.className="topup-option"+(i===1?" popular":""); o.innerHTML=`<b>Add ${money(a)}</b><span>${Math.floor(a/local.price)} OTPs</span>`;
      o.onclick=()=>{state.balance+=a; const us=getUsers(); const idx=us.findIndex(u=>u.id===currentUser.id); us[idx].balances[local.code]=state.balance; saveUsers(us); render(); els.topupModal.classList.add("hidden"); toast("Funded");};
      els.topupOptions.appendChild(o);
    });
    if(!state.active){els.activeOrder.classList.add("hidden");return;}
    els.activeOrder.classList.remove("hidden"); els.orderService.textContent=state.active.icon+" "+state.active.name; els.phoneNumber.textContent=state.active.phone; els.orderStatus.textContent=selected.name;
  ...
  els.registerForm.onsubmit=e=>{
    e.preventDefault();
    const email=$("regEmail").value.trim().toLowerCase(), p=$("regPass").value, p2=$("regPass2").value;
    if(p!==p2) return toast("Passwords don't match");
    if(p.length<6) return toast("Min 6 chars");
    const users=getUsers(); if(users.find(u=>u.email===email)) return toast("Email exists");
    const nu={id:Date.now().toString(),email,password:p,balances:{}}; LOCAL_CURRENCIES.forEach(c=>nu.balances[c.code]=c.topups[1]);
    users.push(nu); saveUsers(users); toast("Account created!"); showApp(nu);
  };
  els.loginForm.onsubmit=e=>{
    e.preventDefault();
    const email=$("loginEmail").value.trim().toLowerCase(), p=$("loginPass").value;
    const u=getUsers().find(x=>x.email===email && x.password===p);
    if(!u) return toast("Wrong email or password");
    toast("Logged in!"); showApp(u);
  };

  els.services.onclick=e=>{
    ...
    const phone=selected.prefix+" "+Math.floor(7000000000+Math.random()*999999999);
    state.active={id:Date.now().toString(),name:b.dataset.id,icon:"💬",phone,otp:null,expiresAt:Date.now()+900000};
    const us=getUsers(); const i=us.findIndex(u=>u.id===currentUser.id); us[i].balances[local.code]=state.balance; saveUsers(us);
    localStorage.setItem(`otphub_active_${currentUser.id}`,JSON.stringify(state.active));
    render(); startTimer();
    setTimeout(()=>{state.active.otp=Math.floor(100000+Math.random()*900000).toString(); localStorage.setItem(`otphub_active_${currentUser.id}`,JSON.stringify(state.active)); render(); toast("OTP Received!");},4000);
  };
  ...
  fetch("https://ipapi.co/json/").then(r=>r.json()).then(d=>{
    const map={NG:"nigeria",US:"usa",GB:"uk",CA:"canada",GH:"ghana",KE:"kenya",IN:"india"};
    const code=map[d.country_code]||"nigeria"; local=LOCAL_CURRENCIES.find(c=>c.code===code)||LOCAL_CURRENCIES[0];
    $("localBadge").textContent=`📍 ${local.flag} ${local.name} • Prices in ${local.currency}`; render();
  }).catch(()=>{});
